...
export default function OopTopicPage() {
  ...
  const trackLabels: Record<string, { label: string; color: string }> = {
    principles: { label: "Core Principles", color: "text-emerald-400" },
    design: { label: "Design & Patterns", color: "text-sky-400" },
    practice: { label: "Interview Practice", color: "text-amber-400" },
  };
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <div>
        ...
        >
          <ChevronLeft className="h-3.5 w-3.5" /> Back to OOP
        </Link>
      ...
      {loading ? (
        ...
        <LessonList topic={topic} progress={progress} baseHref="/oop" />
      )}
    </div>
  );
}
